  ...
  initialize(model) {
    ...
    model.call = async () => {
      ...
      if (
        ...
        Object.keys(model.query).length > 0
      ) {
      ...
      let url = model.params.reduce(
        (prev, curr) =>
          prev.replace(new RegExp(`{${curr.name}}`, "g"), curr.value),
        `${model.host}${model.table}${suffix}${queryParams}`
      );
      ...
      if (
        ...
        (typeof model.defaultHeaders == "object" &&
          Object.keys(model.defaultHeaders).length > 0)
      ) {
        callObject.headers = {
          ...model.headers,
      ...
      const request = require("axios")(callObject).then(
